# jacobmei.eth 授權簽名流程（建議）

## 目標
在不暴露主錢包私鑰的前提下，讓文章顯示：
- `Verified by signer.jacobmei.eth (delegate of jacobmei.eth)`
...
## Step 2. 建立 ENS 子網域
在 ENS 管理介面為 `jacobmei.eth` 建立子網域：
- 名稱：`signer`
- 完整名：`signer.jacobmei.eth`
...
PUBLIC_SIGNER_ENS_NAME=signer.jacobmei.eth
...
- 前端文章頁狀態顯示 `Verified by signer.jacobmei.eth (delegate of jacobmei.eth)`
