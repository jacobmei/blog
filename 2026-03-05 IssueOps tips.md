...
因為現在透過 AI 開發實在太方便，只要環境跑得動，只要 Token 敢花，大概都能開發出一套不錯的機制，但也因為這樣，環境有越來越複雜的傾象，每次最耗時的不是核心邏輯，而是那個「為了點幾下」搞出來的 UI，弄完才發現功能根本用不到幾次。

我在 GitHub 上翻了不少現成專案，但沒有一個剛好符合我的需求：**大範圍搜尋 + AI 幫我先篩一遍 + 手動挑選幾篇就好** ，最後決定：**完全不寫前端，嘗試直接用 GitHub Issue 當操作介面 XD
